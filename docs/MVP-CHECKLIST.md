...
- [ ] Стратегии сбора (выбрать при реализации):
  - [ ] (A) До импорта — собственный pre-flight: ffprobe / расширение / размер → предсказать проблемы
  - [ ] (B) После импорта — diff `project.rootItem` до/после → вычислить что НЕ появилось
  ...
  - [ ] (D) Снять `suppressUI=true` в диагностическом режиме → Premiere покажет свои диалоги → юзер сам увидит причину (но это ломает UX для нормального импорта)

### 26.3. Взаимодействие с §14.6 suppressUI
- [ ] `suppressUI=true` сейчас глушит per-file dialogs от Premiere (кодеки, битые файлы). Это обязательно для batch — иначе юзер кликает OK 20 раз
- [ ] Audit должен дать ту же информацию, что пользователь получил бы от этих диалогов, но в структурированном виде и без блокировки UI
- [ ] Если §26 реализован — флаг остаётся, но юзер ничего не теряет
